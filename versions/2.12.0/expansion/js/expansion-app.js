...
function renderMethodSolve(root) {
  const { a, op, b, selectedMethod, solution, currentStep } = methodsScreenState;
  ...
  stepsContainer.className = 'solve-steps-container';

  for (let i = 0; i <= currentStep && i < solution.steps.length; i++) {
    const step = solution.steps[i];
    const stepDiv = document.createElement('div');
    stepDiv.className = `solve-step ${i === currentStep ? 'current' : 'completed'}`;
    stepDiv.innerHTML = `
  ...
  if (currentStep < solution.steps.length - 1) {
    controls.innerHTML = `
      ...
      <button class="btn btn-secondary" id="show-all-btn">Show All Steps</button>
    `;
  } else {
    controls.innerHTML = `
  ...
  if (currentStep < solution.steps.length - 1) {
    document.getElementById('next-step-btn').addEventListener('click', () => {
      methodsScreenState.currentStep++;
      renderScreen('methods');
    });

    document.getElementById('show-all-btn').addEventListener('click', () => {
      methodsScreenState.currentStep = solution.steps.length - 1;
      renderScreen('methods');
    });
  } else {
